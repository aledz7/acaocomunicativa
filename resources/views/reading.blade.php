			...
			<div class="max-w-2xl mx-auto bg-gray-50 py-2 px-4 mb-10" style="background: #f4f7f6">
				<div x-data='{sendMail:false}' class="text-center sm:flex sm:items-center sm:justify-between p-2 rounded sm:space-x-4">
					<div class="text-sm py-2 text-gray-700">
						<span class="text-gray-400">{{ date('d/M/y', strtotime($reading->date))}}</span> - <b class="text-black">{{config('app.name')}}</b>
					</div>
					<div class="flex justify-center sm:justify-end mx-auto space-x-2">
						<a 
							href="https://api.whatsapp.com/send?text={{ config('app.url')}}/{{$reading->slug}}  - {{ config('app.name')}} - {{$reading->title}}"
						...
						<a rel="https://www.linkedin.com/shareArticle?mini=true&url={{config('app.url')}}&title={{$reading->title}}&summary=undefined&source=" 
							title="Share this page on Facebook" 
							onclick='openPopUp($(this).attr("rel"), $(this).attr("title"))'
							class="text-gray-700 mx-1 hover:text-blue-700 cursor-pointer transition duration-300 ease-in-out" 
						...
						<a rel="https://twitter.com/home?status={{config('app.url')}}%2F{{$reading->slug}}" 
							title="Share this page on Facebook" 
							onclick='openPopUp($(this).attr("rel"), $(this).attr("title"))'
							class="text-gray-700 mx-1 hover:text-blue-700 cursor-pointer transition duration-300 ease-in-out" 
							aria-label="Share on Instagram">
						...
						@livewire('send-to-mail',['type'=>$type,'post_id'=>$reading->id,'title'=>$reading->title,'short_text'=>$reading->short_text])
					</div>
				</div>
			</div>
		</div>
