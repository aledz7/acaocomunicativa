	...
	<div class="space-y-4">
	    ...
	    @foreach( $news as $new )
	        <div class="bg-white px-3 py-2 flex items-center">
	            <div class="pr-6">
	            	@if( $new->cover )
	                	<img src="{{asset('/storage/'.$new->cover)}}" alt="" class="w-12">
	                @else
	                	<img src="{{asset('/img/no-image.png')}}" alt="" class="w-12">
	                @endif
	            </div>
	            <div class="flex-1">
	                <a href="{{route('admin.news.edit',$new)}}" class="flex flex-col">
	                    <p class="font-bold text-sm">{{substr($new->title,0,60)}} </p>
	                    <p class="text-xs text-gray-600">{{ substr($new->short_text,0,150)}}</p>
	                </a>
	            </div>
	            ...
	            <div x-data='{options:false,modal:false}' class="flex justify-between">
	            	<div class="space-x-2 flex">
	                	<a href='{{route("reading",$new->slug)}}' target="_blank">Ver no site </a>
		                <a href="{{route('admin.news.edit',$new)}}" ><button class="bg-blue-600 text-white px-2 rounded text-sm">Editar</button></a>
		                <div class="relative" >
	                        ...
	                        <div class="p-10 ">
	                            ...
	                            <div class="flex justify-between py-2 px-3 mt-4 bg-gray-100 rounded">
	                            	...
	                            	<div>
	                                	...
	                                	<button wire:loading.remove wire:target='sendTo' class="bg-blue-600 px-2 py-1 rounded text-white" wire:click='sendTo({{$new->id}})'>Enviar</button>
	                            	</div>
	                            </div>
	                        </div>
	                    </div>
