<div>
    <div class="row">
        ...
        <div class="col-md-5  mb-4 d-flex justify-content-center align-items-center cursor-pointer" style="cursor: pointer;" onclick="getElementById('cover').click()">
            ...
            @if( $cover )
                ...
                @enderror
            @elseif( $health->cover )
                <img wire:loading.remove wire:target='cover' src="{{asset('storage/'.$health->cover)}}" alt="" class="w-100">
            @else
        ...
        <div class="col-md-7">
            <div class="pb-4">
                <input type="text" wire:model='health.title' class="form-control"  placeholder="Título da Matéria" value="" required="">
                <span class="@if( strlen($health->title) > 60 ) text-danger @endif font-sm">
                    Recomendado 60 caracteres: {{strlen($health->title)}} 
                    @if( strlen($health->title) > 60 )Não tem problema passar, só não é recomendado. @endif
                </span>
                @error('health.title') <span class="text-danger font-sm pull-right">{{$message}}</span> @enderror
            </div>
            <div class="pb-4">
                <input type="text" wire:model='health.slug' class="form-control"  placeholder="link" value="" required="">
                <span class="@if( strlen($health->slug) > 60 ) text-danger @endif font-sm">
                    Recomendado 60 caracteres: {{strlen($health->slug)}} 
                    @if( strlen($health->slug) > 60 )Não tem problema passar, só não é recomendado. @endif
                </span>
                @error('health.slug') <span class="text-danger font-sm pull-right">{{$message}}</span> @enderror
            </div>
            
            <div class="pb-4">
                <input type="date" wire:model.defer='health.date' class="form-control" value="{{date('Y-m-d')}}" required="">
                @error('health.date') <span class="text-danger font-sm pull-right">{{$message}}</span> @enderror
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="pb-4">
                <input type="text" wire:model='health.short_text'  class="form-control"  placeholder="Chamada" value="" required="">
                <span class="@if( strlen($health->short_text) > 150 ) text-danger @endif font-sm">
                    Recomendado 150 caracteres: {{strlen($health->short_text)}} 
                    @if( strlen($health->short_text) > 150 )Não tem problema passar, só não é recomendado. @endif
                </span>
                @error('health.short_text') <span class="text-danger font-sm pull-right">{{$message}}</span> @enderror
            </div>
        </div>
    </div>
    <div class="row" wire:ignore>
        <div class="col-md-12">
            <textarea  name='text' wire:model='health.text' id="summernote"></textarea>
        </div>
    </div>
            ...
            $('#summernote').summernote({
                ...
                callbacks: {
                    onBlur: function(contents) {
                        ...
                        @this.set('health.text', textareaValue);
                    }
                }
            });
        });
