<div>
    <div class="space-y-4">
        <div class="mb-6 bg-white rounded px-3 py-3 text-right">
            <a href="{{route('admin.healths.create')}}" class="bg-blue-600 px-3 py-1 rounded text-white">
        ...
        @foreach( $healths as $health )
            <div class="bg-white px-3 py-2 flex items-center">
                <div class="pr-6">
                    @if( $health->cover )
                        <img src="{{asset('/storage/'.$health->cover)}}" alt="" class="w-12">
                    @else
                        <img src="{{asset('/img/no-image.png')}}" alt="" class="w-12">
                    @endif
                </div>
                <div class="flex-1">
                    <a href="{{route('admin.healths.edit',$health)}}" class="flex flex-col">
                        <p class="font-bold text-sm">{{substr($health->title,0,60)}} </p>
                        <p class="text-xs text-gray-600">{{ substr($health->short_text,0,150)}}</p>
                    </a>
                </div>
                ...
                <div x-data='{options:false,modal:false}' class="flex justify-between">
                    <div class="space-x-2 flex">
                        <a href='{{route("reading",$health->slug)}}' target="_blank">Ver no site </a>
                        <a href="{{route('admin.healths.edit',$health)}}" ><button class="bg-blue-600 text-white px-2 rounded text-sm">Editar</button></a>
                        <div class="relative" >
                    ...
                    <div x-show='modal' class="absolute bg-black bg-opacity-50 left-0 right-0 top-0 bottom-0 p-20" style="display: none;">
                        <div x-show.transition='modal' @='modal = false'  class="bg-white rounded shadow-2xl w-6/12 mx-auto" >
                            ...
                            <div class="p-10 ">
                                ...
                                <div>
                                    @foreach( $newsLetterList as $key=>$profession )
                                ...
                                <div class="flex justify-between py-2 px-3 mt-4 bg-gray-100 rounded">
                                    ...
                                    <div>
                                        ...
                                        <button wire:loading.remove wire:target='sendTo' class="bg-blue-600 px-2 py-1 rounded text-white" wire:click='sendTo({{$health->id}})'>Enviar</button>
                                    </div>
                                </div>
                            </div>
                        </div>
