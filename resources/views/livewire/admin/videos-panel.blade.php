<div>
	<div class="space-y-4">
		<div class="mb-6 bg-white rounded px-3 py-3 text-right">
	        <a href="{{route('admin.videos.create')}}" class="bg-blue-600 px-3 py-1 rounded text-white">
	    ...
	    @foreach( $videos as $video )
	        <div class="bg-white px-3 py-2 flex items-center">
	            <div class="pr-6">
	            	@if( $video->cover )
	                	<img src="{{asset('/storage/'.$video->cover)}}" alt="" class="w-12">
	                @else
	                	<img src="{{asset('/img/no-image.png')}}" alt="" class="w-12">
	                @endif
	            </div>
	            <div class="flex-1">
	                <a href="{{route('admin.videos.edit',$video)}}" class="flex flex-col">
	                    <p class="font-bold text-sm">{{$video->title}} </p>
	                    <p class="text-xs text-gray-600">{{ substr($video->short_text,0,100)}}</p>
	                </a>
	            </div>
	            ...
	            <div class="flex-1 flex flex-col">
	                ...
	                <div>
	                    @foreach( $video->categories as $category)
	                        <span class="bg-gray-100 rounded px-2 text-xs">{{$category->name}}</span>
	                    @endforeach
	                </div>
	            </div>
	            <div x-data='{options:false,modal:false}' class="flex justify-between">
	            	<div class="space-x-2 flex">
	            		@if($video->slug)
	                		<a href='{{route("reading",$video->slug)}}' target="_blank">Ver no site </a>
	                	@else
	                		<span>Problema no link</span>
	                	@endif
		                <a href="{{route('admin.videos.edit',$video)}}" ><button class="bg-blue-600 text-white px-2 rounded text-sm">Editar</button></a>
		                <div class="relative" >
	                ...
	                <div x-show='modal' class="absolute bg-black bg-opacity-50 left-0 right-0 top-0 bottom-0 p-20" style="display: none;">
	                    <div x-show.transition='modal' @='modal = false'  class="bg-white rounded shadow-2xl w-6/12 mx-auto" >
	                        ...
	                        <div class="p-10 ">
	                            ...
	                            <div>
	                            	@foreach( $videosLetterList as $key=>$profession )
	                            ...
	                            <div class="flex justify-between py-2 px-3 mt-4 bg-gray-100 rounded">
	                            	...
	                            	<div>
	                                	...
	                                	<button wire:loading.remove wire:target='sendTo' class="bg-blue-600 px-2 py-1 rounded text-white" wire:click='sendTo({{$video->id}})'>Enviar</button>
	                            	</div>
	                            </div>
	                        </div>
	                    </div>
