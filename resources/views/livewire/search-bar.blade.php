<div x-data='{searchModel : false}' class="float-right">
    <div class="px-3 py-1 hover:text-actionblue transition ease-in-out duration-200 ease-in-out">
	    <svg @click='searchModel = true' onclick='$("#search-bar").focus()' class="w-6 h-6 cursor-pointer" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg>
	</div>

	
	<div x-show.transition='searchModel' class="fixed z-10 top-0 left-0 bg-white mx-auto top-0 w-full" style="display: none;">
		<div class="max-w-6xl mx-auto h-screen flex flex-col pb-10">
			<div class="text-right px-6 pt-3">
				<button @click='searchModel = false' class="text-gray-600 font-bold text-xl px-3 active:outline-none focus:outline-none border rounded">X</button>
			</div>	
			<div class="relative">
				<img src="{{ asset('img/marca-dagua.png') }}" class="opacity-5 absolute left-0 " style="top: -250px; left: -400px">
			</div>
			<div>
				<div class="text-3xl  mb-8 sm:flex">
					<div class="px-4   flex-1">
						<input wire:model='search' id='search-bar' type="text" class="border-b border-gray-300 w-full focus:outline-none" autofocus="autofocus" placeholder="Pesquisar por...">
					</div>
				</div>
			</div>
			<div class="flex-1 overflow-scroll px-4">
				<div> 
					<div class="text-2xl font-bold mb-2">
						Notícias
						<div class="border-b-2 border-actionblue w-20"></div>
					</div>
					<div class="overflow-scroll">
						@if( $news->count() == 0  )
							<p class="w-full p-4 text-center bg-gray-100 block my-6	">Sem notícias</p>
						@else
							@foreach( $news as $new)
								 <div class="hover:bg-gray-50 py-4  border-b border-dashed border-gray-300 group">
								 	<a href='{{route("reading",["slug"=>$new->slug ])}}' class="flex" title="{{ $new->title }} - {{ config('app.name') }}"  alt="{{ $new->title }} - {{ config('app.name') }}"> 	
								 		<div class="w-36 py-1">
								 			@if( $new->cover )
								     			<img src="{{asset('/storage/'.$new->cover)}}"  class="mx-auto rounded" title="{{ $new->title }} - {{ config('app.name') }}"  alt="{{ $new->title }} - {{ config('app.name') }}">
								     		@else
								     			<img src="data:image/png;base64,"  class="mx-auto rounded"  title="{{ $new->title }} - {{ config('app.name') }}"  alt="{{ $new->title }} - {{ config('app.name') }}"> 
								     		@endif
								 		</div>
								 		<div class=" px-4 flex-1">
								 			<h2 class="font-bold group-hover:text-actionblue transition">{{$new->title}}</h2>
								 			<h3 class="text-sm text-gray-600 font-normal">
								 				{{ $new->short_text }}
								 			</h3>
								 			<div class="text-sm py-2 text-gray-700 ">
								 				<span class="text-gray-400">{{ date('d/M/y')}}</span> - <b>{{config('app.name')}}</b>
								 			</div>
								 		</div>
								 	</a>
								 </div>
							@endforeach
						@endif
					</div>
				</div>
				<div class="mt-8">
					<div class="text-2xl font-bold mb-10">
						Vídeos
						<div class="border-b-2 border-actionblue w-20"></div>
					</div>
					@if( $videos->count() == 0  )
						<p class="w-full p-4 text-center bg-gray-100 block my-6	">Sem Vídeos</p>
					@else
						@foreach( $videos as $video)
							 <div class="hover:bg-gray-50 py-4  border-b border-dashed border-gray-300 group">
							 	<a href='{{route("reading",["slug"=>$video->slug ])}}' class="flex"  title="{{ $video->title }} - {{ config('app.name') }}"  alt="{{ $video->title }} - {{ config('app.name') }}"> 	
							 		<div class="w-36 py-1">
							 			@if( $video->cover )
							     			<img src="{{asset('/storage/'.$video->cover)}}"  class="mx-auto rounded"  title="{{ $video->title }} - {{ config('app.name') }}"  alt="{{ $video->title }} - {{ config('app.name') }}">
							     		@else
							     			<img src="data:image/png;base64,"  class="mx-auto rounded"  title="{{ $video->title }} - {{ config('app.name') }}"  alt="{{ $video->title }} - {{ config('app.name') }}"> 
							     		@endif
							 		</div>
							 		<div class=" px-4 flex-1">

							 			<h2 class="font-bold group-hover:text-actionblue transition">{{$video->title}}</h2>
							 			<h3 class="text-sm text-gray-600">
							 				{{$video->short_text}}
							 			</h3>
							 			<div class="text-sm py-2 text-gray-700 ">
							 				<span class="text-gray-400">{{ date('d/M/y')}}</span> - <b>{{config('app.name')}}</b>
							 			</div>
							 		</div>
							 	</a>
							 </div>
						@endforeach
					@endif
				</div>
				<div class="mt-8">
					<div class="text-2xl font-bold mb-10">
						#saude
						<div class="border-b-2 border-actionblue w-20"></div>
					</div>
					@if( $healths->count() == 0  )
						<p class="w-full p-4 text-center bg-gray-100 block my-6	">Sem #saude</p>
					@else
						@foreach( $healths as $health)
							 <div class="hover:bg-gray-50 py-4  border-b border-dashed border-gray-300 group">
							 	<a href='{{route("reading",["slug"=>$health->slug ])}}' class="flex"  title="{{ $health->title }} - {{ config('app.name') }}"  alt="{{ $health->title }} - {{ config('app.name') }}"> 	
							 		<div class="w-36 py-1">
							 			@if( $health->cover )
							     			<img src="{{asset('/storage/'.$health->cover)}}"  class="mx-auto rounded"  title="{{ $health->title }} - {{ config('app.name') }}"  alt="{{ $health->title }} - {{ config('app.name') }}">
							     		@else
							     			<img src="data:image/png;base64,"  class="mx-auto rounded"  title="{{ $health->title }} - {{ config('app.name') }}"  alt="{{ $health->title }} - {{ config('app.name') }}"> 
							     		@endif
							 		</div>
							 		<div class=" px-4 flex-1">

							 			<h2 class="font-bold group-hover:text-actionblue transition">{{$health->title}}</h2>
							 			<h3 class="text-sm text-gray-600">
							 				{{$health->short_text}}
							 			</h3>
							 			<div class="text-sm py-2 text-gray-700 ">
							 				<span class="text-gray-400">{{ date('d/M/y')}}</span> - <b>{{config('app.name')}}</b>
							 			</div>
							 		</div>
							 	</a>
							 </div>
						@endforeach
					@endif
				</div>
			</div>
		</div>
	</div>
</div>
